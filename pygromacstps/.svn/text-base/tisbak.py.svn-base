    ...
    def pickConfigurationLastAccepted(self,dirnumber,pathnumber):
        ...
        if self.paths[pathnumber].options.runoptions["shootfrominterface"] == 0:
            ...
            for kr in range(0,sum-2):
                distance = fabs(self.paths[pathnumber].lastAcceptedFullTrajectory[distopi][kr][1] - self.paths[pathnumber].interface)
                if distance < mindist:
                    mindist = distance
                    rn = kr
...
#        if (self.cores<len(todolist)):
#            toploop = self.cores-1 
#        else:
#            toploop = len(todolist)
...
#                        self.log.log.info(str(i) + " " + str(processlist[i].poll()) + " " + str(len(self.distparser.data)) + " " +  str(self.distparser.data[-1]) + " " + str(float(self.distparser.data[-1][self.stablestates.gdistDirection])) + " " + str(self.stablestates.states))
#                        pathfinished = self.paths[i].checkFinishedTIS(float(self.distparser.data[-1][self.stablestates.gdistDirection]),self.stablestates.states)
#                        if pathfinished or len(self.distparser.data) > self.paths[i].options.runoptions["maxlength"]:
    ...
    def shootingQueueDynamic(self):
        """
        Jeroen: Shooting Queue dynamically spread over several nodes using MPI and or openmp
        """
    ...
    def checkAllTisPathsAccepted(self):
        ...
        for i in self.kernels.kernelPaths:
            ...
            self.log.log.debug(str(i) + "  " +str(self.paths[i].fullTrajectory[0][0]))
            self.log.log.debug(str(i) + "  " +str(self.paths[i].fullTrajectory[0][-1]))
            ...
            if accepted:
                ...
                for opi in range(len(self.orderparameters.op)):
                    self.paths[i].lastAcceptedFullTrajectory.append(self.paths[i].fullTrajectory[opi][:])
                    self.paths[i+1].lastAcceptedFullTrajectory.append(self.paths[i].fullTrajectory[opi][:])
